...
namespace XCS
{
    class Program
    {
        ...
        public static void Experiment(Environment Env, Population P)
        {
            ...
            //正解act のリスト
            List<char> ActionList = Env.GetActionList();
            ...
            {
                Configuration.ConvergentedVT[i] = new StdList(DistinctDataList[i], ActionList[i]);
                //Configuration.ConvergentedVT[i * 4 + 1] = new StdList(i, '1');
            }
            for (int i = 0; i < DistinctDataNum; i++)
            {
                Configuration.Stdlist[i] = new StdList(DistinctDataList[i], ActionList[i]);
