...
namespace XCS
{
    class DiffVarianceEnv_2_2 : Environment
    {
        ...
        // Environment作成
        public DiffVarianceEnv_2_2(int Length, double NoiseWidth)
        {
            this.Length = Length;
            this.Number = 2;    // 2進数
        ...
        override public State GetState()
        {
            ...
            if (state.state.Substring(0, 4) == "0000")
            {
                return GetState();
        ...
        public override double GetReward()
        {
            ...
            if (L4.Count != 0)
            {
                ...
                switch (IndexOfS4)
                {
                    case 0:
                        //1000## return 0-|e|
                        return 0.0 +this.MakeNoise2();

                    case 1:
                        //0100## return 0+|e|
                        return  300+ this.MakeNoise2();
                    case 2:
                        //0010## return 1000-|e|
                        return
                            800 + MakeNoise1();
        ...
        protected double MakeNoise2()
        {
            double A = Configuration.MT_P.NextDoublePositive();
            double B = Configuration.MT_P.NextDoublePositive();

            return this.NoiseWidth * 2 * (Math.Sqrt(-2 * Math.Log(A, Math.E)) * Math.Sin(2 * Math.PI * B));
        }
    }
}