...
namespace XCS
{
    class DiffVarianceEnv_2_1 : Environment
    {
        ...
        // Environment作成
        public DiffVarianceEnv_2_1(int Length, double NoiseWidth)
        {
            this.Length = Length;
            this.Number = 2;    // 2進数
        ...
        protected double MakeNoise2()
        {
            ...
            return Math.Abs(this.NoiseWidth *2* (Math.Sqrt(-2 * Math.Log(A, Math.E)) * Math.Sin(2 * Math.PI * B)));
        }
    }
}